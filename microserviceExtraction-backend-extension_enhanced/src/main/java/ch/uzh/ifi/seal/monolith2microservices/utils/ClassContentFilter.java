...
public class ClassContentFilter implements FilterInterface {
    ...
    public List<String> filterFileContent(String rawFileContent) {
        ...
        for(String token: tokens){
            if(!StopWords.JAVA_KEYWORDS.contains(token) && !StopWords.CSHARP_KEYWORDS.contains(token) && !StopWords.RUBY_KEYWORDS.contains(token) && !StopWords.PYTHON_KEYWORDS.contains(token) && !StopWords.LICENSE_KEYWORDS.contains(token) && (token.length() > 1)){
